@php
    use Filament\Models\Contracts\HasCurrentTenantLabel;use Filament\Support\Facades\FilamentView;use Illuminate\Database\Eloquent\Model;use function Filament\Support\prepare_inherited_attributes;$currentTenant = filament()->getTenant();
...
{{ FilamentView::renderHook('panels::tenant-menu.before') }}
...
>
    <x-slot name="trigger">
        <button
                ...
                @endif
                type="button"
                class="group flex w-full items-center justify-center gap-x-3 rounded-lg p-2 text-sm font-medium outline-none transition duration-75 hover:bg-gray-100 focus:bg-gray-100"
        >
            ...
            <x-filament::icon
                    ...
                    class="ms-auto h-5 w-5 shrink-0 text-gray-400 transition duration-75 group-hover:text-gray-500 group-focus:text-gray-500"
            />
        </button>
    </x-slot>
...
{{ FilamentView::renderHook('panels::tenant-menu.after') }}
