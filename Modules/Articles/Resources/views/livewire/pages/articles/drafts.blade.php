...
@section('content')
    ...
    <div class="bg-neutral px-6 py-1 border-t border-b border-gray-200 sm:flex sm:items-center sm:justify-between">
        <nav class="flex space-x-8 py-2" aria-label="Global">
            <a href="{{ route('articles.drafts') }}" class="bg-white text-base-text-color inline-flex items-center rounded-md py-2 px-3 font-medium"
               aria-current="page">
                <x-library::icons.icon name="fa-regular fa-pen-to-square" size="w-5 h-5" class="pr-2"/>
                Drafts</a>
            <a href="{{ route('articles.published') }}" class="text-base-text-color hover:bg-white hover:text-base-text-color inline-flex items-center rounded-md py-2 px-3
            font-medium">
                <x-library::icons.icon name="fa-duotone fa-newspaper" size="w-5 h-5" class="pr-2"/>
                Published</a>
        </nav>
    ...
    <div class="mt-4">
        <div class="masonry sm:masonry-1 md:masonry-2">
            ...
            @empty
                <li class="p-4 bg-secondary rounded-md text-base-text-color">No articles to show</li>
            @endforelse
        </div>
