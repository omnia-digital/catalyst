    ...
    @auth()
        @if(auth()->user()->id != $model->id )
            <div class="inline-flex items-center text-md">
                ...
                @else
                    <button type="button" class="inline-flex items-center px-4 py-2 rounded-full bg-primary text-white-text-color text-sm tracking-wide font-medium hover:opacity-75" wire:click="follow">
                        <span>Follow</span>
                    </button>
                @endif
            </div>
