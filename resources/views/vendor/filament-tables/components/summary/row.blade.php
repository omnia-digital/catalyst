...
@php
    ...
    use Filament\Tables\Enums\RecordCheckboxPosition;use function Filament\Support\prepare_inherited_attributes;
@endphp
        ...
        @if (($loop->first || $extraHeadingColumn || $groupsOnly || ($loop->iteration > $headingColumnSpan)) && ($placeholderColumns || $column->hasSummary()))
            ...
            >
                @if ($loop->first && (! $extraHeadingColumn) && (! $groupsOnly))
                    <span
                            class="flex px-3 py-4 text-sm font-medium text-gray-950"
                    >
                        {{ $heading }}
                    </span>
