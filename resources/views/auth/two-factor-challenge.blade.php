        ...
        <div x-data="{ recovery: false }">
            ...
            <form method="POST" action="{{ route('two-factor.login') }}">
                ...
                <div class="mt-4" x-show="! recovery">
                    <x-jet-label for="code" value="{{ \Trans::get('Code') }}" />
                    <x-jet-input id="code" class="block mt-1 w-full" type="text" inputmode="numeric" name="code" autofocus x-ref="code" autocomplete="one-time-code" />
                </div>

                <div class="mt-4" x-show="recovery">
                    <x-jet-label for="recovery_code" value="{{ \Trans::get('Recovery Code') }}" />
                    <x-jet-input id="recovery_code" class="block mt-1 w-full" type="text" name="recovery_code" x-ref="recovery_code" autocomplete="one-time-code" />
                </div>

                <div class="flex items-center justify-end mt-4">
                    <button type="button" class="text-sm text-base-text-color hover:text-dark-text-color underline cursor-pointer"
                                    x-show="! recovery"
                                    x-on:click="
                                        recovery = true;
