<div class="py-12">
                                ...
                                @forelse ($episodeTemplates as $episodeTemplate)
                                    <x-table.row :loop="$loop" wire:key="{{ $episodeTemplate->id }}" class="">
                                        <x-table.cell class="font-medium text-gray-900">
                                            ...
                                            <span>
                                                @if ($defaultEpisodeTemplateId === $episodeTemplate->id)
                                                    <x-heroicon-s-star class="inline-flex items-center w-4 h-4 text-blue-500 ml-2"/>
                                                @endif
                                            </span>
                                        </x-table.cell>
                                        ...
                                        <x-table.cell class="text-right font-medium flex-1 items-center space-x-2">
                                            <x-form.button-link :to="route('episode-templates.update', $episodeTemplate)" size="p-2" secondary>
                                                <x-heroicon-o-pencil class="w-4 h-4"/>
                                            </x-form.button-link>

                                            @if ($defaultEpisodeTemplateId !== $episodeTemplate->id)
                                                <x-form.button-link wire:click="setDefault('{{ $episodeTemplate->id }}')" size="p-2" secondary>
                                ...
                                @empty
                                    <x-table.empty>
                                        <div class="text-center">
                                            <p class="text-center text-gray-600 text-base my-4">No episode templates matched the given criteria.</p>
                                            <x-form.button wire:click="$toggle('createEpisodeTemplateModalOpen')" secondary size="py-1 px-4">
    ...
    <x-jet-dialog-modal wire:model="createEpisodeTemplateModalOpen">
        ...
        <x-slot name="content">
            ...
            <div class="mt-4">
                ...
                <x-input.filepond id="episode-thumbnail" wire:model.defer="thumbnail"/>
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$toggle('createEpisodeTemplateModalOpen')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-jet-secondary-button>

            <x-jet-button class="ml-2" wire:click="createEpisodeTemplate" wire:loading.attr="disabled">
