<div x-on:click.stop="">
    <div class="inline-flex items-center text-md">
        ...
        @else
            <button wire:click.prevent.stop="showLoginModal('{{ route('social.posts.show', $model) }}')" type="button" class="inline-flex space-x-2 text-light-text-color hover:text-base-text-color">
                <x-heroicon-o-refresh class="h-5 w-5" aria-hidden="true"/>
                <span class="font-medium text-dark-text-color">{{ $model->reposts ?? '' }}</span>
                <span class="sr-only">Repost</span>
            </button>
