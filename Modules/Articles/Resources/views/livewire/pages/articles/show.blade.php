@extends('catalyst-catalyst-social::livewire.layouts.pages.default-page-layout')

@section('content')
    @empty($article)
        <x-library::heading.2>No Article found</x-library::heading.2>
    @else
        ...
        <div class="w-full">
            <div class="col-span-4 card">
                @if ($article->image)
                    <div>
                        <img class="rounded-lg rounded-b w-full object-cover max-h-96 bg-neutral-dark flex-shrink-0"
                ...
                <div class="px-6">
                    <div class="flex items-center space-x-2 mt-6">
                        <x-library::heading.3
                                class="text-2xl text-dark-text-color sm:text-4xl hover:underline font-bold">{{ $article->title }}</x-library::heading.3>
                        @empty(!$article->is_verified)
                    ...
                    <div class="flex justify-start items-center my-2">
                        <div class="flex">
                            @if (is_null($article->published_at))
                                <p class="italic text-base-text-color text-md"><span
                                            class="text-primary-400">Draft</span>
                                    created {{ $article->created_at->format('M d, Y') }}</p>
                            @else
                                <x-heroicon-o-calendar class="w-5 h-5"/>
                                <p class="ml-2 text-base-text-color text-md">{{ $article->published_at->format('M d, Y') }}</p>
                            @endif
                        </div>
                        ...
                        @can('update', $article)
                            <div class="ml-auto flex justify-end items-center">
                                <x-library::button.link href="{{ route('articles.edit', $article->id) }}" size=""
                                                        class="text-primary border-none rounded-none shadow-none hover:underline">
                                    Edit
                    ...
                    @if ($article->url)
                        <a href="{{ $article->url }}" target="_blank"
                           class="bg-primary hover:shadow-lg rounded-lg px-4 py-2 text-xl text-white-text-color inline-flex items-center space-x-2">
                    ...
                    <div>
                        <livewire:catalyst-catalyst-social::partials.post-actions wire:key="article-actions-{{ $article->id }}"
                                                                :post="$article" :show-comment-button="false"
            ...
            @if ($article->isParent())
                <div class="py-4 justify-center w-full xl:w-3/4 2xl:1/2 m-auto max-w-post-card-max-w">
                    <div class="max-w-post-card-max-w">
                        ...
                        <livewire:catalyst-catalyst-social::comment-section :post="$article"
