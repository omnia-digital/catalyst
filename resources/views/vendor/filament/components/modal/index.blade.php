@php
    use Filament\Support\Enums\Alignment;use Filament\Support\View\Components\Modal;use function Filament\Support\get_color_css_variables;use function Filament\Support\is_slot_empty;
@endphp
                ...
                @if ($heading || $header)
                    <div
                            @class([
                                ...
                                'sticky top-0 z-10 border-b border-gray-200 bg-white bg-white pb-6' => $stickyHeader,
                        ...
                        @else
                            @if ($icon)
                                ...
                                >
                                    <div
                                            @class([
                                                'rounded-full bg-custom-100',
                                                match ($alignment) {
                                                    Alignment::Left, Alignment::Start, 'left', 'start' => 'p-2',
                                                    Alignment::Center, 'center' => 'p-3',
                                                },
                                            ])
                                            style="{{ get_color_css_variables($iconColor, shades: [100, 500]) }}"
                                    >
                                        <x-filament::icon
                                                :alias="$iconAlias"
                                                :icon="$icon"
                                                ...
                                                @style([
                                                    get_color_css_variables($iconColor, shades: [400, 600]) => $iconColor !== 'gray',
                                                ])
                                        />
                                    </div>
                                </div>
                ...
                @if ((! is_slot_empty($footer)) || (is_array($footerActions) && count($footerActions)) || (! is_array($footerActions) && ! is_slot_empty($footerActions)))
                    <div
                            @class([
                                ...
                                'sticky bottom-0 border-t border-gray-200 bg-white py-5' => $stickyFooter,
