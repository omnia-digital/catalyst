        ...
        <div class="w-full">
            <div class="col-span-4 card">
                ...
                <div class="px-6">
                    <div class="flex items-center space-x-2 mt-6">
                        <x-library::heading.3 class="text-2xl text-dark-text-color sm:text-4xl hover:underline font-bold">{{ $article->title }}</x-library::heading.3>
                    ...
                    <div class="flex justify-start items-center my-2">
                        <div class="flex">
                            @if (is_null($article->published_at))
                                <p class="italic text-base-text-color text-md"><span class="text-primary-400">Draft</span> created {{ $article->created_at->format('M d, Y') }}</p>
                            @else
                        ...
                        @can('update', $article)
                            <div class="ml-auto flex justify-end items-center">
                                <x-library::button.link href="{{ route('articles.edit', $article->id) }}" size="" class="text-primary border-none rounded-none shadow-none hover:underline">
                                    Edit
                    ...
                    <div>
                        <livewire:social::partials.post-actions wire:key="article-actions-{{ $article->id }}" :post="$article" :show-comment-button="false" :show-bookmark-button="true"/>
                    </div>
                </div>
            </div>
            ...
            @if ($article->isParent())
                <div class="py-4 justify-center w-full xl:w-3/4 2xl:1/2 m-auto max-w-post-card-max-w">
                    ...
                    <livewire:social::comment-section :post="$article" :type="\Modules\Social\Enums\PostType::ARTICLE"/>
                    </div>
                </div>
            @endif
        </div>
