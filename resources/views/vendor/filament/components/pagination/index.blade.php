...
@php use function Filament\Support\format_number; @endphp
@props([
    'paginator',
    'pageOptions' => [],
...
>
    ...
    @if (! $isSimple)
        <span
                class="hidden text-sm font-medium text-gray-700 md:inline"
        >
            {{
                trans_choice(
                    ...
                    $paginator->total(),
                    [
                        'first' => format_number($paginator->firstItem() ?? 0),
                        'last' => format_number($paginator->lastItem() ?? 0),
                        'total' => format_number($paginator->total()),
                    ],
                )
            }}
        </span>
    ...
    @if ((! $isSimple) && $paginator->hasPages())
        <ol
                class="hidden justify-self-end rounded-lg bg-white shadow-sm ring-1 ring-gray-950/10 md:flex"
        >
            @if (! $paginator->onFirstPage())
