                        ...
                        <div class="flex w-full mt-1 space-x-1 overflow-x-scroll h-40" style="scrollbar-width: thin;">
                            <div class="flex h-36 pl-2">
                                @foreach ($team->sampleImages() as $media)
                                    <span class="w-40 h-32 mr-2 mt-2 flex justify-center items-center relative pr-1 last:pr-0 cursor-pointer hover:ring-4 hover:ring-neutral-dark active:ring-4 active:ring-neutral-dark focus:ring-4 focus:ring-neutral-dark {{ ($media->id === $displayID) ? 'ring-2 ring-neutral-dark' : '' }}"
                    ...
                    <div class="flex flex-col flex-1 bg-primary rounded">
                        ...
                        <div class="p-4 flex flex-col flex-1">
                            ...
                            <div class="text-xs grid grid-cols-4 grid-rows-4 gap-1 items-center">
                                ...
                                {{-- <span class="col-span-1 text-gray-400 text-2xs uppercase">Reviews:</span>
                                <div class="col-span-3 flex items-center space-x-2">
                                    {{-- Review score algorithm not set
                                        <div class="bg-black flex items-center rounded-md mr-1 p-1">
                                        <div class="flex items-center text-white-text-color text-xs font-semibold">
                                            <x-heroicon-s-star class="w-4 h-4" />
                                            {{ $team->reviewScore ?? '3758' }}
                                        </div>
                                    </div>
                                    <span class="text-gray-400 text-2xs">{{ $team->reviewStatus ?? 'Overwhelmingly Positive' }} ({{ /* $team->reviews()->count */'296,418' }})</span>
                                </div> --}}
                            </div>
            ...
            <div class="lg:grid lg:grid-rows-1 lg:grid-cols-3 lg:gap-4">
                @if ($team->content)
                    <div class="flex-1 flex flex-col">
                        <p class="text-base-text-color font-semibold">{{ \Trans::get('About this Team') }}</p>
