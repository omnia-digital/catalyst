    ...
    <nav class="w-full flex justify-between">
        ...
        <div class="flex w-1/2 justify-center items-center">
            <a href="{{ route('home') }}" class="mx-4" aria-label="Home">
                <img class="h-8 w-auto sm:h-10" src="{{ asset('images/logo-white.png') }}" alt="{{ config('app.name') }}">
            </a>
        </div>
        <div class="flex w-1/4 justify-end items-center">
            @auth()
            @else
                <a href="{{ route('login') }}" class="mr-4 inline-flex items-center text-base leading-6 font-medium text-white transition duration-150 ease-in-out">
                    Login
                </a>
            @endauth
