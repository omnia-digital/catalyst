<div>
    <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div class="-ml-4 mb-4 flex justify-between items-center flex-wrap sm:flex-nowrap">
            ...
            <div class="ml-4 mt-4 flex-shrink-0">
                <button x-data x-on:click="$dispatch('show-slide-over')" type="button" class="relative inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                    Edit Player
                </button>
            </div>
        </div>
    ...
    <x-slide-over>
        ...
        <div class="flex-shrink-0 py-2 flex justify-end">
            <x-jet-danger-button wire:click="$toggle('deletePlayerModalOpen')" wire:loading.attr="disabled">
                {{ __('Delete Player') }}
            </x-jet-danger-button>
        ...
        <div class="space-y-6 pt-4 pb-5">
            ...
            <div>
                ...
                <x-input.text type="number" wire:model.defer="layout.columns" id="columns"/>
                <x-input.error for="layout.columns"/>
            </div>
            <div>
                <x-input.label value="Videos Per Page"/>
                <x-input.text type="number" wire:model.defer="layout.video_per_page" id="video-per-page"/>
                <x-input.error for="layout.video_per_page"/>
            </div>
            <div>
                <x-input.label value="Background Color"/>
                <x-input.text type="color" wire:model.defer="layout.background_color" id="background-color"/>
                <x-input.error for="layout.background_color"/>
                <x-input.help value="You won't see the background changes in the preview, it only applies on your website."/>
            </div>
            <div>
    ...
    <x-jet-confirmation-modal wire:model="deletePlayerModalOpen">
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$toggle('deletePlayerModalOpen')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-jet-secondary-button>

            <x-jet-danger-button class="ml-2" wire:click="deletePlayer" wire:loading.attr="disabled">
                {{ __('Delete') }}
            </x-jet-danger-button>
        </x-slot>
