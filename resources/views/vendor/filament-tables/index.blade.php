@php
    ...
    use Filament\Tables\Actions\Action;use Filament\Tables\Actions\ActionGroup;use Filament\Tables\Actions\BulkAction;use Filament\Tables\Columns\Column;use Filament\Tables\Enums\ActionsPosition;
    use Filament\Tables\Enums\FiltersLayout;
    use Filament\Tables\Enums\RecordCheckboxPosition;use Filament\Tables\Filters\BaseFilter;use Illuminate\Contracts\Pagination\LengthAwarePaginator;use Illuminate\Contracts\Pagination\Paginator;use Illuminate\Database\Query\Builder;use function Filament\Support\prepare_inherited_attributes;
...
>
    <x-filament-tables::container>
        ...
        x-bind:hidden="! (@js($hasHeader) || (selectedRecords.length && @js(count($bulkActions))))"
                ...
                class="fi-ta-header-ctn divide-y divide-gray-200"
        >
            @if ($header)
                {{ $header }}
        ...
        <div
                ...
                @endif
                @class([
                    'fi-ta-content relative divide-y divide-gray-200 overflow-x-auto',
        ...
        >
            @if (($content || $hasColumnsLayout) && ($records !== null) && count($records))
                @if (! $isReordering)
                    ...
                    <div
                            @class([
                                'flex items-center gap-4 gap-x-6 bg-gray-50 px-4 sm:px-6',
                ...
                @else
                    <x-filament::grid
                            ...
                            @class([
                                ...
                                'gap-y-px bg-gray-200' => ! $contentGrid,
                            ])
                    >
                        @php
                            $previousRecord = null;
                        ...
                        @foreach ($records as $record)
                            ...
                            @if ($recordGroupTitle !== $previousRecordGroupTitle)
                                ...
                                <x-filament-tables::group.header
                                        ...
                                        @class([
                                            'col-span-full',
                                            '-mx-4 w-[calc(100%+2rem)] border-y border-gray-200 first:border-t-0 sm:-mx-6 sm:w-[calc(100%+3rem)]' => $contentGrid,
                                        ])
                            ...
                            <div
                                    ...
                                    @endif
                                    @class([
                                        'relative h-full bg-white transition duration-75',
                                        'hover:bg-gray-50' => ($recordUrl || $recordAction) && (! $contentGrid),
                                        'hover:bg-gray-50' => ($recordUrl || $recordAction) && $contentGrid,
                                    ...
                                    {{ ($contentGrid ? '\'bg-gray-50'' : 'null'bg-gray-50 before:absolute before:start-0 before:inset-y-0 before:w-0.5 before:bg-primary-600'') . ': isRecordSelected(null'' . $recordKey . '\')' }},
                                    {{ $contentGrid ? '\'bg-white': ! isRecordSelected(null'' . $recordKey . '\')' : '\'\':\'\'' }},
                                }"
                            >
                                @php
                                ...
                                >
                                    ...
                                    @foreach ($columns as $column)
                                        ...
                                        <x-filament-tables::cell
                                                ...
                                                :attributes="
                                                prepare_inherited_attributes($column->getExtraCellAttributeBag())
                                                    ->class([
